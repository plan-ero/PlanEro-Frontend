...
import { Download, CheckCircle, XCircle, AlertCircle, Smartphone, Shield } from "lucide-react";
    ...
    try {
      ...
      if (hasServiceWorker && navigator.serviceWorker) {
        navigator.serviceWorker.getRegistration()
          .then(registration => {
            if (registration) {
              console.log('[PWA] Service worker already registered:', registration);
              setSwReady(true);
            } else {
        ...
        <CardContent className="space-y-4">
          {isInstalled ? (
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-semibold mb-2">PWA Not Fully Supported</h3>
              <p className="text-muted-foreground mb-4">
                Your browser or environment doesn't fully support Progressive Web Apps
              </p>
              <div className="text-left text-sm space-y-2 bg-muted p-4 rounded-lg">
                <p className="font-medium">Requirements:</p>
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  <li>Service Worker support: {checks.hasServiceWorker ? '✓' : '✗'}</li>
                  <li>Manifest file: {checks.hasManifest ? '✓' : '✗'}</li>
                  <li>Secure context (HTTPS/localhost): {checks.isSecureContext ? '✓' : '✗'}</li>
                </ul>
              </div>
            </div>
          ) : deferredPrompt ? (
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-semibold mb-2">Install Available via Browser</h3>
              <p className="text-muted-foreground mb-4">
                Chrome supports PWA installation! Use the browser menu to install.
              </p>
              <div className="text-left text-sm space-y-3 bg-muted p-4 rounded-lg">
                <div>
                  <p className="font-medium mb-1">Chrome Desktop:</p>
                  <p className="text-muted-foreground">Click the <strong>⋮</strong> menu → "Install Planero..." or look for the install icon in the address bar</p>
                </div>
                <div>
                  <p className="font-medium mb-1">Chrome Mobile:</p>
                  <p className="text-muted-foreground">Tap the <strong>⋮</strong> menu → "Add to Home screen" or "Install app"</p>
                </div>
                <div>
                  <p className="font-medium mb-1">Edge:</p>
                  <p className="text-muted-foreground">Click the <strong>⋯</strong> menu → "Apps" → "Install Planero"</p>
                </div>
              </div>
              <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-900">
                  <strong>Service Worker:</strong> {swReady ? '✓ Ready' : '⏳ Loading...'}
                </p>
              </div>
            </div>
          )}
