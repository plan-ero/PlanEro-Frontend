...
export async function GET(request: NextRequest) {
  try {
    ...
    if (vendorId) {
      const response = await fetch(`${BACKEND_URL}/services?vendorId=${vendorId}`)

      if (!response.ok) {
        console.error(`Backend error (${response.status}): Failed to fetch services for vendor ${vendorId}`)
    ...
    try {
      ...
      for (const vendor of vendors) {
        try {
          const servicesResponse = await fetch(`${BACKEND_URL}/services?vendorId=${vendor.id}`)
          ...
          console.error(`Error fetching services for vendor ${vendor.id}:`, err)
