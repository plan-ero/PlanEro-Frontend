...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!session) {
      ...
      console.log("=== API Route /api/upload/image POST END (UNAUTHORIZED) ===")
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!session) {
      console.log("ERROR: Unauthorized - no session")
      console.log("=== API Route /api/upload/image DELETE END (UNAUTHORIZED) ===")
      return NextResponse.json(
        { error: "Unauthorized" },
        { status: 401 }
      )
    ...
    if (!url) {
      ...
      console.log("=== API Route /api/upload/image DELETE END (BAD REQUEST) ===")
      return NextResponse.json(
